...
async function tryRename() {
  ...
  btn.disabled = false;
  btn.innerHTML =
    '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> Rename';
}
...
(function () {
  ...
  document
    .querySelectorAll(".api-url")
    .forEach((el) => (el.textContent = base));
  document
    .querySelectorAll(".api-url-py")
    .forEach((el) => (el.textContent = '"' + full + '"'));
  document
    .querySelectorAll(".api-url-ps")
    .forEach((el) => (el.textContent = '"' + full + '"'));
})();
