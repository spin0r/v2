...
async function getPage(url: string, session: AxiosInstance): Promise<string | null> {
...
export class ViperGirlsDownloader {
  ...
  extractPostsWithTitles(html: string): PostData[] {
    ...
    $("li.postbitlegacy").each((_, el) => {
      ...
      for (const [tag, cls] of [["h2", "title"], ["div", "title"]]) {
        ...
        if (found.length) { title = found.text().trim(); break; }
      }
      if (!title) {
  ...
  async scrapeThread(
    ...
    for (const n of Object.keys(pageResults).map(Number).sort((a, b) => a - b)) {
      let posts = pageResults[n];
      if (titleFilter)
        posts = posts.filter(
          (p) => p.title && p.title.toLowerCase().includes(titleFilter.toLowerCase()),
        );
  ...
  _parseSearchResults(html: string): [SearchResult[], number, number] {
    ...
    const parseDate = (text: string): number => {
      ...
      if (relMatch) {
        ...
        const date = new Date();
        if (dayWord.toLowerCase() === "yesterday") date.setDate(date.getDate() - 1);
        let h = parseInt(hour);
        if (ampm) {
    ...
    if (stats.length) {
      ...
      if (m) {
        const [s, e, tot] = m.slice(1).map((x) => parseInt(x.replace(/,/g, "")));
        const pp = e - s + 1;
        if (pp > 0) { perPage = pp; totalPages = Math.ceil(tot / pp); }
      }
    }
...
export class AdultPhotoSetsScraper {
  ...
  async searchAll(query: string): Promise<ApsResult[]> {
    ...
    try { tagResults = await this.getTagResults(query); } catch {}
    const seenUrls = new Set<string>();
    const merged: ApsResult[] = [];
    for (const r of [...allSearch, ...tagResults]) {
      if (!seenUrls.has(r.url)) { seenUrls.add(r.url); merged.push(r); }
    }
    return merged;
  }

  async search(query: string, page = 1): Promise<[ApsResult[], number]> {
    const allowedCats = ["24","33","34","17","15","32","10","12","16","13","14"];
    const params = new URLSearchParams([
      ["do", "search"], ["subaction", "search"],
      ["search_start", String(page - 1)], ["full_search", "1"],
      ["result_from", String((page - 1) * this.RESULTS_PER_PAGE + 1)],
      ["story", query], ["sortby", "date"], ["resorder", "desc"],
      ["searchdate", "0"], ["beforeafter", "after"],
      ...allowedCats.map((c): [string, string] => ["catlist[]", c]),
    ]);
    const html = await this._post(`${this.BASE_URL}/index.php?do=search`, params.toString());
    if (!html) return [[], 1];
  ...
  private _parseSearch(html: string): [ApsResult[], number] {
    ...
    $("article.story").each((_, el) => {
      ...
      if (thumbUrl && !thumbUrl.startsWith("http")) thumbUrl = this.BASE_URL + thumbUrl;
      results.push({ title: link.text().trim(), url, category: catDiv.text().trim(), thumb: thumbUrl });
    });
    let totalPages = 1;
    const nav = $("div.navigation div.pages");
    if (nav.length) {
      const nums = [...(nav.html() || "").matchAll(/list_submit\((\d+)\)/g)].map((m) => parseInt(m[1]));
  ...
  async getTagResults(query: string): Promise<ApsResult[]> {
    const tagSlug = encodeURIComponent(query.replace(/\b\w/g, (c) => c.toUpperCase()));
  ...
  private async _fetchTagPages(baseUrl: string): Promise<ApsResult[]> {
    const fetchPage = async (n: number): Promise<[ApsResult[], number]> => {
      ...
      $("article.story").each((_, el) => {
        ...
        if (!link.length) return;
        const catDiv = $(el).find("div.category");
        results.push({ title: link.text().trim(), url: link.attr("href") || "", category: catDiv.text().trim() });
      });
      let total = 1;
      const nav = $("div.navigation div.pages");
      if (nav.length) {
        const nums = [...(nav.html() || "").matchAll(/\/page\/(\d+)\//g)].map((m) => parseInt(m[1]));
