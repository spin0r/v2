...
export function bindCardEvents(appEl) {
  ...
  appEl.querySelectorAll("[data-fetch-idx]").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
        ...
        try {
          ...
          if (data.ok && data.threadData) {
            ...
            if (posts.length === 1) {
                  ...
                  if (progress.type === "phase") {
                    ...
                    if (card) {
                      ...
                      if (
                        prog &&
                        info.phase === "extracting" &&
                        !prog.querySelector(".progress-counter")
                      ) {
                        const span = document.createElement("span");
                        span.className = "progress-counter";
                        span.dataset.fetchProgress = String(idx);
              ...
              render();
              toast(
                `✓ ${r.title?.slice(0, 40)} — ${result.extracted || 0}/${result.total || 0}`,
                "success",
              );
            } else {
      ...
      try {
        ...
        const data = await apiFetchStream(
          state.tab,
          id,
          state.query,
          (progress) => {
            ...
            if (progress.type === "phase") {
              ...
              if (card) {
                ...
                if (label)
                  label.textContent =
                    info.phase === "extracting" ? "Extracting" : "Scraping…";
                const prog = card.querySelector(".inline-progress");
                if (
                  prog &&
                  info.phase === "extracting" &&
        ...
        render();
        toast(
          `✓ ${r.title?.slice(0, 40)} — ${data.extracted || 0}/${data.total || 0}`,
          "success",
        );
      } catch (err) {
        state.fetchingCards.delete(idx);
        state.completedCards.set(idx, {
          ok: false,
          error: err.message,
          title: r.title,
          sourceUrl: r.url,
  ...
  appEl.querySelectorAll(".inline-extract-btn").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      try {
        ...
        render();
        toast(
          `✓ ${result.title?.slice(0, 40)} — ${data.extracted || 0}/${data.total || 0}`,
          "success",
        );
      } catch (err) {
        state.fetchingCards.delete(key);
        state.completedCards.set(key, {
          ok: false,
          error: err.message,
          title: scraped.posts[postIdx]?.title,
        });
        render();
  ...
  appEl.querySelectorAll("[data-view-completed]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      const result =
        state.completedCards.get(raw) ||
        state.completedCards.get(parseInt(raw));
      if (result) {
  ...
  appEl.querySelectorAll("[data-retry-card-idx]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      requestAnimationFrame(() => {
        const newBtn = appEl.querySelector(
          `[data-card-idx="${cardIdx}"][data-post-idx="${postIdx}"]`,
        );
        if (newBtn) newBtn.click();
      });
    });
