...
export function bindModalEvents(appEl) {
  ...
  if (threadNextBtn) {
    threadNextBtn.addEventListener("click", () => {
      if (
        state.threadData &&
        state.threadPage < state.threadData.pages.length - 1
      ) {
        state.threadPage++;
        render();
      }
  ...
  appEl.querySelectorAll(".extract-post-btn").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      state.fetchingThreadPosts.set(gidx, {
        phase: "extracting",
        extracted: 0,
        total: 0,
      });
      ...
      try {
        ...
        render();
        toast(
          `✓ ${postTitle?.slice(0, 40)} — ${data.extracted || 0}/${data.total || 0}`,
          "success",
        );
      } catch (err) {
        state.fetchingThreadPosts.delete(gidx);
        state.completedThreadPosts.set(gidx, {
          ok: false,
          error: err.message,
          title: postTitle,
          sourceUrl: d.url,
  ...
  if (reextractFailedBtn)
    reextractFailedBtn.addEventListener("click", async () => {
      ...
      reextractFailedBtn.innerHTML =
        '<div class="spinner" style="width:12px;height:12px;border-width:1.5px"></div> Retrying…';
      try {
        ...
        if (result.newlyRecovered > 0) {
          toast(
            `✓ Recovered ${result.newlyRecovered} images! Now ${result.extracted}/${result.total}`,
            "success",
          );
        } else {
          toast(
            `No additional images recovered (${result.extracted}/${result.total})`,
            "error",
          );
        }
      } catch (err) {
