...
import { apiDirectFetch, apiThreadPostExtractStream } from "../api.js";
import { fetchRssFeed } from "../views/rss.js";

export function bindRssEvents(appEl) {
  // RSS events
  ...
  // RSS "Get images" — scrape thread then show post picker or extract directly
  appEl.querySelectorAll("[data-rss-fetch]").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
      const idx = parseInt(btn.dataset.rssFetch);
      const entry = state.rssEntries[idx];
      if (!entry || !entry.link) return;
      const rssKey = `rss-${idx}`;
      if (state.fetchingCards.has(rssKey)) return;

      state.fetchingCards.set(rssKey, {
        phase: "scraping",
        extracted: 0,
        total: 0,
      });
      ...
      try {
        ...
        if (data.ok && data.threadData) {
          ...
          if (posts.length === 1) {
              ...
              (progress) => {
                ...
                if (progress.type === "phase") {
                  ...
                  const el = document.querySelector(
                    `[data-fetch-progress="${rssKey}"]`,
                  );
                  if (el)
                    el.textContent = `${progress.extracted}/${progress.total}`;
                }
              },
            );
            state.fetchingCards.delete(rssKey);
            state.completedCards.set(rssKey, {
              ...result,
              title: result.title || entry.title,
              sourceUrl: result.sourceUrl || entry.link,
            });
            render();
            toast(
              `✓ ${entry.title?.slice(0, 40)} — ${result.extracted || 0}/${result.total || 0}`,
              "success",
            );
          } else {
            ...
            render();
            toast(
              `${posts.length} posts found — pick one to extract`,
              "success",
            );
          }
        } else {
  ...
  appEl.querySelectorAll("[data-rss-extract]").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
      const idx = parseInt(btn.dataset.rssExtract);
      const pi = parseInt(btn.dataset.rssExtractPi);
      const rssKey = `rss-${idx}`;
      ...
      state.fetchingCards.set(postKey, {
        phase: "extracting",
        extracted: 0,
        total: post?.count || 0,
      });
      ...
      try {
        ...
        const entry = state.rssEntries[idx];
        state.completedCards.set(postKey, {
          ...result,
          title: result.title || entry?.title,
          sourceUrl: result.sourceUrl || entry?.link,
        });
        render();
        toast(
          `✓ Extracted ${result.extracted || 0}/${result.total || 0}`,
          "success",
        );
      } catch (err) {
        state.fetchingCards.delete(postKey);
        state.completedCards.set(postKey, { ok: false, error: err.message });
        render();
  ...
  appEl.querySelectorAll("[data-rss-retry]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const idx = parseInt(btn.dataset.rssRetry);
      const rssKey = `rss-${idx}`;
      state.completedCards.delete(rssKey);
      state.scrapedCards.delete(rssKey);
      render();
      // Re-trigger fetch
      const fetchBtn = appEl.querySelector(`[data-rss-fetch="${idx}"]`);
      if (fetchBtn) fetchBtn.click();
    });
  });

  // RSS post-level retry buttons
  appEl.querySelectorAll("[data-rss-post-retry]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const idx = parseInt(btn.dataset.rssPostRetry);
      const pi = parseInt(btn.dataset.rssPostRetryPi);
      const postKey = `rss-${idx}-${pi}`;
      state.completedCards.delete(postKey);
      render();
      // Re-trigger extract
      const extractBtn = appEl.querySelector(
        `[data-rss-extract="${idx}"][data-rss-extract-pi="${pi}"]`,
      );
