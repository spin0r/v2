...
export async function handleVgScrape(params: URLSearchParams, res: ServerResponse): Promise<void> {
  ...
  for (const results of vgCache.values()) {
    found = (results as Record<string, unknown>[]).find((r) => r.sgenId === id) ?? null;
    if (found) break;
  }
  if (!found) return sendJSON(res, 404, { error: "ID not found – run search first" });

  const downloader = new ViperGirlsDownloader();
  const [pagesData, totalPages] = await downloader.scrapeThread(found.url as string);
...
export async function handleVgFetch(params: URLSearchParams, res: ServerResponse): Promise<void> {
  const id = params.get("id") || "";
  const query = params.get("q") || "";
  ...
  if (stream) { startSSE(res); sendSSE(res, "phase", { phase: "scraping" }); }
  ...
  if (stream) sendSSE(res, "phase", { phase: "extracting", total: allLinks.length });

  const onProgress = stream ? (p: unknown) => sendSSE(res, "progress", p) : null;
  const result = await extractAndUpload(allLinks, found.title as string, found.url as string, query || null, onProgress);
  if (result.ok) addToHistory(result);

  if (stream) { sendSSE(res, "done", result); res.end(); }
  else sendJSON(res, 200, result);
}

export async function handleApsFetch(params: URLSearchParams, res: ServerResponse): Promise<void> {
  const id = params.get("id") || "";
  const query = params.get("q") || "";
  const stream = params.get("stream") === "1";
  if (!id) return sendJSON(res, 400, { error: "Missing id" });
  ...
  for (const results of apsCache.values()) {
    found = (results as Record<string, unknown>[]).find((r) => r.apsId === id) ?? null;
    if (found) break;
  }
  if (!found) return sendJSON(res, 404, { error: "ID not found – run search first" });
  ...
  if (stream) sendSSE(res, "phase", { phase: "extracting", total: links.length });

  const onProgress = stream ? (p: unknown) => sendSSE(res, "progress", p) : null;
  const result = await extractAndUpload(links, found.title as string, found.url as string, query || null, onProgress);
  if (result.ok) addToHistory(result);
...
export async function handleDirectFetch(params: URLSearchParams, res: ServerResponse): Promise<void> {
...
export async function handleThreadExtract(params: URLSearchParams, res: ServerResponse): Promise<void> {
  const threadId = params.get("threadId") || "";
  const gidx = parseInt(params.get("postIndex") || "0");
  const stream = params.get("stream") === "1";
  if (!threadId) return sendJSON(res, 400, { error: "Missing threadId" });
  if (!threadCache.has(threadId)) return sendJSON(res, 404, { error: "Thread not found or expired. Please search again." });

  const threadData = threadCache.get(threadId) as { pages: { posts: { title?: string; links: string[] }[] }[]; url: string; searchQuery: string | null };
  let post: { title?: string; links: string[] } | null = null;
  let cur = 0;
  outer: for (const page of threadData.pages) {
    for (const p of page.posts) {
      if (cur === gidx) { post = p; break outer; }
      cur++;
    }
  }
  ...
  if (stream) { startSSE(res); sendSSE(res, "phase", { phase: "extracting", total: post.links.length }); }

  try {
    const onProgress = stream ? (p: unknown) => sendSSE(res, "progress", p) : null;
    const result = await extractAndUpload(post.links, title, threadData.url, threadData.searchQuery, onProgress);
    if (result.ok) addToHistory(result);
    if (stream) { sendSSE(res, "done", result); res.end(); }
    else sendJSON(res, 200, result);
  } catch (err) {
    if (stream) { sendSSE(res, "error", { error: (err as Error).message }); res.end(); }
    else sendJSON(res, 500, { error: (err as Error).message });
  }
}

export async function handleReExtract(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const body = await readBody(req);
  let parsed: Record<string, unknown>;
  try { parsed = JSON.parse(body); }
  catch { return sendJSON(res, 400, { error: "Invalid JSON" }); }

  const { failedLinks, previousUrls, indexedFailedLinks, indexedUrls, title, sourceUrl, searchQuery } = parsed as {
    failedLinks: string[];
    previousUrls?: string[];
  ...
  if (!failedLinks?.length) return sendJSON(res, 400, { error: "No failed links to retry" });
  ...
  (extractor as unknown as { client: { defaults: { timeout: number } } }).client.defaults.timeout = 25000;
  ...
  if (hasIndexedData) for (const [idx, url] of Object.entries(indexedUrls!)) urlMap[idx] = url;
    ...
    const results = await Promise.allSettled(
      chunk.map((entry) => {
        ...
        return extractor.extractDirectUrl(link).then((u) => {
          ...
          if (u) { if (hostMatch) hostCounts[hostMatch] = (hostCounts[hostMatch] || 0) + 1; }
          else { failedHostsMap[hostName] = (failedHostsMap[hostName] || 0) + 1; stillFailed.push(link); stillFailedIndexed.push({ index, link }); }
          return { link, u, index };
        }).catch(() => {
          const hostMatch = IMAGE_HOSTS.find((h) => link.includes(h));
          const hostName = hostMatch ? hostMatch.split(".")[0] : "unknown";
          failedHostsMap[hostName] = (failedHostsMap[hostName] || 0) + 1;
          stillFailed.push(link); stillFailedIndexed.push({ index, link });
          return { link, u: null, index };
        });
      }),
    );
    for (const r of results)
      if (r.status === "fulfilled" && r.value.u) urlMap[r.value.index] = r.value.u;
  }

  let allUrls: string[];
  let newlyRecovered: number;
  if (hasIndexedData) {
    allUrls = Object.keys(urlMap).map(Number).sort((a, b) => a - b).map((i) => urlMap[i]);
  ...
  const totalOriginal = hasIndexedData
    ? Object.keys(indexedUrls!).length + (indexedFailedLinks?.length ?? failedLinks.length)
    : prevUrls.length + failedLinks.length;

  if (!allUrls.length) {
    return sendJSON(res, 200, { ok: false, error: "All re-extraction attempts failed", title: title || "", sourceUrl: sourceUrl || "", total: totalOriginal, extracted: 0, failed: totalOriginal, failedLinks: stillFailed, indexedFailedLinks: stillFailedIndexed.length > 0 ? stillFailedIndexed : undefined, failedHosts: failedHostsMap, directUrls: [], indexedUrls: undefined, pasteUrl: null });
  }
  ...
  const finalResult: Record<string, unknown> = {
    ...
    extracted: allUrls.length, failed: stillFailed.length,
