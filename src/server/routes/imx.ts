...
export async function handleImxUpload(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  sendJSON(res, 200, { ok: pbResult.success, total: imageUrls.length, uploaded: successResults.length, extracted: directUrls.length, galleryUrl: galleryId ? `https://imx.to/g/${galleryId}` : null, previewUrls: directUrls.slice(0, 5), pasteUrl: pbResult.success ? pbResult.url : null, pasteError: pbResult.success ? null : pbResult.error });
}
