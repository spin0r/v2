...
export async function extractAndUpload(
  ...
  console.log('[extractAndUpload] Received title:', title);
  ...
  for (
    ...
    (
      retryExtractor as unknown as { client: { defaults: { timeout: number } } }
  ...
  if (!directUrls.length) {
    ...
    return {
      ...
      error:
        total === 0
          ? 'No image links found in this post'
          : `Images not found${failedDetail ? ` — failed hosts: ${failedDetail}` : ''}`,
      title,
      sourceUrl,
      total,
      failed: failedCount,
