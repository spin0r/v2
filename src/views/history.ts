...
import { svgIcon, toast } from "../utils.ts";
import { apiHistory } from "../api.ts";
...
function renderHistoryCard(h: import("../api.ts").HistoryEntry, idx: number): string {
...
function renderHistoryView(skipAnim: boolean): string {
  const header = `
...
async function fetchHistory(page = 1): Promise<void> {
  ...
  try {
    ...
    toast(`History failed: ${(err as Error).message}`, "error");
  }
