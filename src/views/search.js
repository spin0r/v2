...
function renderCard(r, idx) {
  ...
  const idLabel = r.sgenId
    ? `/sgen${r.sgenId}`
    : r.apsId
      ? `/aps${r.apsId}`
      : "";
  const prefixHtml = r.prefix
    ? `<span class="result-prefix">${r.prefix}</span>`
    : "";
  ...
  if (isFetching) {
    ...
    const isImgNotFound =
      errMsg.toLowerCase().includes("images not found") ||
      errMsg.toLowerCase().includes("no image links");
    const hasFailedLinks =
      ok && completedData?.failedLinks && completedData.failedLinks.length > 0;
    actionsHtml = `
      <div class="result-actions">
    ...
    const postsHtml = scrapedData.posts
      .map((p, pi) => {
        ...
        if (postFetchInfo) {
          ...
          const txt =
            phase === "extracting"
              ? `${postFetchInfo.extracted ?? 0}/${postFetchInfo.total ?? "?"}`
              : "Scraping…";
          ...
          const pIsImgNotFound =
            pErrMsg.toLowerCase().includes("images not found") ||
            pErrMsg.toLowerCase().includes("no image links");
...
function renderResults() {
  ...
  const hasCompletedCmds =
    [...state.completedCards.values()].some(
...
function renderThreadView() {
  ...
  const postsHtml = pageData.posts
    .map((post, i) => {
      ...
      const fetchInfo = isFetchingPost
        ? state.fetchingThreadPosts.get(gidx)
        : null;
      const isCompletedPost = state.completedThreadPosts.has(gidx);
      const completedPostData = isCompletedPost
        ? state.completedThreadPosts.get(gidx)
        : null;
      ...
      if (isFetchingPost) {
        ...
        const tErrMsg = !ok
          ? completedPostData?.error || "Extraction failed"
          : "";
        const tIsImgNotFound =
          tErrMsg.toLowerCase().includes("images not found") ||
          tErrMsg.toLowerCase().includes("no image links");
        const tHasFailedLinks =
          ok &&
          completedPostData?.failedLinks &&
          completedPostData.failedLinks.length > 0;
        actionsHtml = `<div class="result-actions">
...
function renderModal() {
  ...
  const isImgNotFoundModal =
    (d.error || "").toLowerCase().includes("images not found") ||
    (d.error || "").toLowerCase().includes("no image links");
